...
class SunatService
{
    ...
    public function getInvoice($company,$documento,$venta,$totales)
    {
        if($venta->pago == 0){
            return (new Invoice())
                ->setUblVersion('2.1')
                ->setTipoOperacion('0101') // Venta - Catalog. 51 0205 / Exportación de Servicios - Servicios que conformen un Paquete Turístico 
                ...
                ->setTipoMoneda($venta->medio->moneda_id == 1 ? 'PEN':'USD') // Sol - Catalog. 02
                ...
                ->setDetails($this->getDetails($venta->detalleventas))
    ...
    public function getNote($company,$documento,$venta,$totales,$note)
    {
        return (new Note())
            ...
            ->setTipoDoc($documento->codSunat) // Factura - Catalog. 01 
            ->setSerie($documento->serie)
            ->setCorrelativo($note->nume_doc)
            ->setFechaEmision(new DateTime($note->fecha)) // Zona horaria: Lima
            ->setTipDocAfectado($venta->document->codSunat)
            ->setNumDocfectado($venta->document->serie.'-'.$venta->nume_doc)
            ->setCodMotivo($note->code_note)
            ->setDesMotivo($note->descripcion)
            ->setTipoMoneda($note->medio->moneda_id == 1 ? 'PEN':'USD') // Sol - Catalog. 02
            ->setCompany($this->getCompany($company))
            ->setClient($this->getClient($venta->cliente))
            ->setMtoOperGravadas($totales['MtoOperGravadas'])
            ->setMtoIGV($totales['MtoIGV'])
