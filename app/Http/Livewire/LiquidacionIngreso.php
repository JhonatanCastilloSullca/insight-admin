...
class LiquidacionIngreso extends Component
{
    ...
    public function mount()
    {
        ...
        $this->agencias=Proveedor::where('categoria_id','11')->get();
    }

    public function updatedfecha($value)
    {
        $detalles=DetalleReserva::whereRelation('reserva','tipo',2)->whereRelation('reserva','proveedor_id',$this->idAgencia)->where('fecha_viaje','<=',$value)->where('pago',0)->get();
    ...
    {
        $detalles=DetalleReserva::whereRelation('reserva','tipo',2)->whereRelation('reserva','proveedor_id',$value)->where('fecha_viaje','<=',$this->fecha)->where('pago',0)->get();
        $this->detalles=$detalles;
        $this->cantidadshow=$this->detalles->where('estado',3)->sum('cantidad');
        $this->cantidadnoshow=$this->detalles->where('estado',2)->sum('cantidad');
