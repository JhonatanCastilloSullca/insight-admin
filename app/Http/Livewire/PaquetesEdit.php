...
class PaquetesEdit extends Component
{
    use WithFileUploads;
    public $editingIndex;
    ...
    public $precioserviciosolesHotel = 0.0;
    public $precioserviciodolaresHotel = 0.0;
    ...
    public $precioserviciosolesVuelo = 0.0;
    public $precioserviciodolaresVuelo = 0.0;
    ...
    public $precioserviciosoles = 0.0;
    public $precioserviciodolares = 0.0;
    public $servicioData = [];
    ...
    public function updatedIdavuelta()
    {
    }
    public function mount($paqueteId = null)
    {
        ...
        if ($paqueteId) {
            ...
            if ($paquete) {
                ...
                $this->imagenprincipal = $paquete->img_principal;
                $this->originalImagenPrincipal = $this->imagenprincipal;
                $this->imagensecundario = $paquete->img_secundario;
                $this->originalImagenSecundario = $this->imagensecundario;
                $this->regularsoles = $paquete->regularsoles;
                $this->regulardolares = $paquete->regulardolares;
                $this->total_soles = $paquete->precio_soles;
                $this->total_dolares = $paquete->precio_dolares;
                $this->descripcion = $paquete->descripcion;
                $this->adulto = $paquete->adulto;

                //PASAJEROS
                foreach ($paquete->pasajeros as $key => $value) {
                    $this->nombres = $value->nombres;
                    $this->genero = $value->genero;
                    $this->celular = $value->celular;
                    $this->email = $value->email;
                    $this->pais_id = $value->pais->id;
                }
                //SERVICIOS
                $this->reservaData = $paquete->detallestours
                    ->map(function ($detalle) {
                        $incluyeservice = $detalle->detallesIncluidos
                            ->map(function ($detalleIncluido) {
                                return [
                                    "id" =>
                                    $detalleIncluido->servicio_incluido_id,
                                    "titulo" =>
                                    $detalleIncluido->servicioIncluido
                                        ->titulo,
                                ];
                            })
                            ->toArray();
                        $noincluyeservice = $detalle->detallesNoIncluidos
                            ->map(function ($detalleNoIncluido) {
                                return [
                                    "id" =>
                                    $detalleNoIncluido->servicio_no_incluido_id,
                                    "titulo" =>
                                    $detalleNoIncluido->servicioNoIncluido
                                        ->titulo,
                                ];
                            })
                            ->toArray();
                        return [
                            ...
                            "noincluyeservice" => $noincluyeservice,
                            "dia_servicio" => $detalle->dia_servicio,
                            "precioserviciosoles" => $detalle->preciosoles,
                            "precioserviciodolares" => $detalle->preciodolares,
                            "tipo" => $detalle->tipo,
                            "adulto" => $detalle->adulto,
                        ];
                    })
                    ->toArray();
                //HOTEL
                $this->reservaDataHotel = $paquete->detalleshoteles
                    ->map(function ($detalle) {
                        $incluyeservice = $detalle->detallesIncluidos
                            ->map(function ($detalleIncluido) {
                                return [
                        ...
                        return [
                            ...
                            "incluyeserviceHotel" => $incluyeservice,
                            "noincluyeserviceHotel" => $noincluyeservice,
                            "precioserviciosolesHotel" => $detalle->preciosoles,
                            "precioserviciodolaresHotel" =>
                            $detalle->preciodolares,
                        ];
                    })
                    ->toArray();
                //VUELOS
                $this->reservaDataVuelo = $paquete->detallesvuelos
                    ->map(function ($detalle) {
                        $incluyeservice = $detalle->detallesIncluidos
                            ->map(function ($detalleIncluido) {
                                return [
                        ...
                        if (count($partes) == 2) {
                            list($desde, $hasta) = explode("/", $partes[0]);
                            list($desdevuelta, $hastavuelta) = explode(
                                "/",
                                $partes[1]
                            );
                        ...
                        return [
                            ...
                            "incluyeserviceVuelo" => $incluyeservice,
                            "noincluyeserviceVuelo" => $noincluyeservice,
                            "precioserviciosolesVuelo" => $detalle->preciosoles,
                            "precioserviciodolaresVuelo" => $detalle->preciodolares,
                            "desde" => $desde,
                            "hasta" => $hasta,
                            "desdevuelta" => $desdevuelta,
                            "hastavuelta" => $hastavuelta,
                            "descripcion" => $detalle->descripcion,
    ...
    {
        return view("livewire.paquetes-edit");
    }
    ...
    public function agregarPasajero()
    {
        ...
        if ($this->imagenpasajero) {
            $nombreimg =
                ...
                $this->num_doc .
                "." .
                $this->imagenpasajero->getClientOriginalExtension();
            $ruta = $this->imagenpasajero->storeAs(
                "img/documentos",
                $nombreimg
            );
            Image::make("storage/" . $ruta)
                ->fit(1600, null, function ($constraint) {
    ...
    public function updatedServicio()
    {
        ...
        $this->precioserviciosoles = $servicio->precio_neto_soles;
        $this->precioserviciodolares = $servicio->precio_neto_dolar;
        if ($servicio && $servicio->incluyes) {
            $this->servicioData = [
                "servicio" => $servicio,
            ];
            $this->incluyeservice = $servicio->incluyes
                ->map(function ($item) {
                    return ["id" => $item->id, "titulo" => $item->titulo];
                })
                ->toArray();
            $this->noincluyeservice = $servicio->noincluyes
                ->map(function ($item) {
                    return ["id" => $item->id, "titulo" => $item->titulo];
                })
                ->toArray();
    ...
    public function updatedServicioHotel()
    {
        ...
        if ($servicio && $servicio->incluyes) {
            ...
            $this->incluyeserviceHotel = $servicio->incluyes
                ->map(function ($item) {
                    return ["id" => $item->id, "titulo" => $item->titulo];
                })
                ->toArray();
            $this->noincluyeserviceHotel = $servicio->noincluyes
                ->map(function ($item) {
                    return ["id" => $item->id, "titulo" => $item->titulo];
                })
                ->toArray();
    ...
    public function updatedServicioVuelo()
    {
        ...
        if ($servicio && $servicio->incluyes) {
            ...
            $this->incluyeserviceVuelo = $servicio->incluyes
                ->map(function ($item) {
                    return ["id" => $item->id, "titulo" => $item->titulo];
                })
                ->toArray();
            $this->noincluyeserviceVuelo = $servicio->noincluyes
                ->map(function ($item) {
                    return ["id" => $item->id, "titulo" => $item->titulo];
                })
                ->toArray();
    ...
    public function EliminarNoIncluyeDetalleHotel($index)
    {
        ...
        $this->noincluyeserviceHotel = array_values(
            $this->noincluyeserviceHotel
        );
    }
    public function EliminarNoIncluyeDetalleVuelo($index)
    {
        ...
        $this->noincluyeserviceVuelo = array_values(
            $this->noincluyeserviceVuelo
        );
    }
        ...
        if ($this->editingIndex !== null) {
            ...
            $this->reservaData[$this->editingIndex] = [
                "servicioData" => $this->servicioData
                    ? $this->servicioData
                    : $this->reservaData[$this->editingIndex]["servicioData"],
        ...
        if ($this->editingIndexHotel !== null) {
            ...
            $this->reservaDataHotel[$this->editingIndexHotel] = [
                "servicioDataHotel" => $this->servicioDataHotel
                    ? $this->servicioDataHotel
                    : $this->reservaDataHotel[$this->editingIndexHotel]["servicioDataHotel"],
        ...
        if ($this->editingIndexVuelo !== null) {
            ...
            $this->reservaDataVuelo[$this->editingIndexVuelo] = [
                "servicioDataVuelo" => $this->servicioDataVuelo
                    ? $this->servicioDataVuelo
                    : $this->reservaDataVuelo[$this->editingIndexVuelo]["servicioDataVuelo"],
                ...
                "precioserviciodolaresVuelo" =>
                $this->precioserviciodolaresVuelo,
    ...
    public function updatePaquete($paqueteId)
    {
        ...
        $ultimoPaquete = Paquete::orderBy("id", "desc")->first();
        $ultimoid = $ultimoPaquete->id + 1;
        if ($this->imagenprincipal !== $this->originalImagenPrincipal) {
            ...
            $nombreArchivo =
                "paquetes/" .
                Str::slug("paquete-" . $paquete->id, "-") .
                "." .
                $this->imagenprincipal->getClientOriginalExtension();
            $this->imagenprincipal->storeAs("paquetes", $nombreArchivo);
            $imagen->save(storage_path("app/public/" . $nombreArchivo));
            $paquete->img_principal = $nombreArchivo;
        } else {
            $paquete->img_principal = $this->imagenprincipal;
        }
        if ($this->imagensecundario !== $this->originalImagenSecundario) {
            ...
            $nombreArchivo =
                "paquetes/" .
                Str::slug("paquete-secundario-" . $paquete->id, "-") .
                "." .
                $this->imagensecundario->getClientOriginalExtension();
            $this->imagensecundario->storeAs("paquetes", $nombreArchivo);
            $imagen->save(storage_path("app/public/" . $nombreArchivo));
            $paquete->img_secundario = $nombreArchivo;
        } else {
            $paquete->img_secundario = $this->imagensecundario;
        }
        ...
        $paquete->save();
        $paquete->pasajeros()->detach();
        $paquete->pasajeros()->attach($pasajero);
        foreach ($paquete->detallestours as $detalle) {
            $detalle->detallesIncluidos()->delete();
            $detalle->detallesNoIncluidos()->delete();
        }
        $paquete->detallestours()->delete();
        ...
        $paquete->detalleshoteles()->delete();
        foreach ($this->reservaDataHotel as $reserva) {
            $detallePaquete = DetallePaquete::create([
                "paquete_id" => $paquete->id,
                "servicio_id" =>
                $reserva["servicioDataHotel"]["servicio"]["id"],
            ...
            if (!empty($reserva["noincluyeserviceHotel"])) {
                foreach ($reserva["noincluyeserviceHotel"]
                    as $noIncluyeService) {
                    DetallePaqueteNoIncluye::create([
                        "detalle_paquete_id" => $detallePaquete->id,
                        "servicio_no_incluido_id" => $noIncluyeService["id"],
        ...
        $paquete->detallesvuelos()->delete();
        foreach ($this->reservaDataVuelo as $reserva) {
            $detallePaqueteData = [
                "paquete_id" => $paquete->id,
                "servicio_id" =>
                $reserva["servicioDataVuelo"]["servicio"]["id"],
            ...
            if ($reserva["idavuelta"]) {
                $detallePaqueteData["descripcion"] =
                    "(" .
                    $reserva["desde"] .
                    " / " .
                    $reserva["hasta"] .
            ...
            if (!empty($reserva["noincluyeserviceVuelo"])) {
                foreach ($reserva["noincluyeserviceVuelo"]
                    as $noIncluyeService) {
                    DetallePaqueteNoIncluye::create([
                        "detalle_paquete_id" => $detallePaquete->id,
                        "servicio_no_incluido_id" => $noIncluyeService["id"],
    ...
    public function savePaquete()
    {
        $validatedData = $this->validate(
            ...
            [
                "reservaData.required" =>
                "El paquete debe tener al menos un servicio",
            ]
        );
        ...
        if ($this->imagensecundario) {
            ...
            $nombreArchivo =
                "paquetes/" .
                Str::slug("paquete-secundario-" . $ultimoid, "-") .
                "." .
                $this->imagensecundario->getClientOriginalExtension();
            $imagen->save(storage_path("app/public/" . $nombreArchivo));
            $paquete->img_secundario = $nombreArchivo;
    ...
    public function openModal()
    {
        if ($this->editingIndex !== null) {
            ...
            $this->privado = $data["tipo"] ? true : false;
    ...
    public function openModalHotel()
    {
        if ($this->editingIndexHotel !== null) {
            ...
            $this->precioserviciodolaresHotel =
                $data["precioserviciodolaresHotel"];
