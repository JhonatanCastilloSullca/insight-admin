...
class CrearSinFecha extends Component
{
    use WithFileUploads;
    ...
    public function registrarReserva()
    {
        ...
        try
        {
            ...
            foreach($this->vuelosreserva as $i => $servicio){
                ...
                if ($detalle) {
                    ...
                    if ($servicio['noches'] != $detalle->equipaje) {
                        $historiaService->addChanges('vuelos' . $i . 'equipaje', $servicio['noches'], $detalle->equipaje);
                    }
                    if ($servicio['precio'] != $detalle->precio) {
        ...
        return redirect()
            ->route("reserva.sinfecha")
            ->with("success", "Reserva Agregado Correctamente.");
    }

    public function render()
    {
        return view('livewire.crear-sin-fecha');
    }
}
