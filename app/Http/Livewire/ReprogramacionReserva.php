...
class ReprogramacionReserva extends Component
{
    public $nombres;
    ...
    public function updatedservicioId($id)
    {
        ...
        if($servicio == ""){
            ...
            $this->precio = $servicio->precios()->wherePivot('moneda_id', 1)->wherePivot('precio_id', 1)->first();
            $this->preciomin = $servicio->precios()->wherePivot('moneda_id', 2)->wherePivot('precio_id', 1)->first();
            $this->precio = $this->preciomin ? $this->preciomin->pivot?->precio : 0;
            $this->preciomin = $this->preciomin ? $this->preciomin->pivot?->precio : 0;
    ...
    public function agregarHotel()
    {
        ...
        $this->validate([
            ...
            'preciohotel' => 'required',
            'moneda_idhotel' => 'required|exists:monedas,id',
        ]);
        $servicio = Servicio::find($this->hotel_id);
        $moneda = Moneda::find($this->moneda_idhotel);
        $fechaInicio = Carbon::parse($this->fecha_viajehotel)->startOfDay();
        $fechaFin = Carbon::parse($this->fecha_viajehotelfin)->startOfDay();
        ...
        $nuevoHotel = [
            ...
            'pax' => $this->paxhotel,
            'fecha_viaje' => $this->fecha_viajehotel,
            'fecha_viajefin' => $this->fecha_viajehotelfin,
    ...
    public function registrarReserva()
    {
        ...
        try
        {
            ...
            foreach ($this->serviciosreserva as $i => $servicio) {
                $detalleanterior = DetalleReserva::find($servicio['detalle_id']);
                $detalleanterior->estado = 3 ;
                ...
                $detalle->fecha_viaje = $servicio['fecha_viaje'];
            ...
            foreach($this->hotelesreserva as $i => $servicio){
                ...
                $detalle->moneda_id = $servicio['moneda_id'];
                $detalle->fecha_viaje = $servicio['fecha_viaje'];
                $detalle->fecha_viajefin = $servicio['fecha_viajefin'] ?? null;
                $detalle->pax = $servicio['pax'];
            ...
            $reservaAnterior->estado = 3;
            $reservaAnterior->save();

            $reprogramacion = new Reprogramacion();
            $reprogramacion->reserva_id = $reserva->id;
            $reprogramacion->user_id = \Auth::user()->id;
            $reprogramacion->reserva_ant_id = $reservaAnterior->id;
            $reprogramacion->fecha = $mytime->toDateTimeString();
            $reprogramacion->comentarios = $this->comentariosRepro;
            $reprogramacion->save();
    ...
    {
        return view('livewire.reprogramacion-reserva');
    }
}
