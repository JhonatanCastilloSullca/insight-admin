<div>
    <div class="breadcrumb-header justify-content-between">
        <div class="left-content">
            <span class="main-content-title mg-b-0 mg-b-lg-1">Editar {{($categoriaint == 1 ? 'Servicio':($categoriaint == 2 ? 'Hotel':($categoriaint == 3 ? 'Vuelo' : 'Servicio')))}}</span>
        </div>
    </div>
    <div class="row">
                ...
                <div class="card-body">
                    <div class="row">
                        <div class="mb-3 col-md-12">
                            <label for="titulo" class="form-label">{{($categoriaint == 1 ? 'Nombre de Servicio':($categoriaint == 2 ? 'Nombre de Hotel':($categoriaint == 3 ? 'Vuelo' : 'Nombre')))}}</label>
                    ...
                    <div class="row">
                        ...
                        <div class="mb-3 col-md-4">
                            ...
                            @else
                                <a wire:click="verImagen('{{ asset($img_principal) }}')">
                                    <img src="{{ asset($img_principal) }}" class="imagen-principal">
                                </a>
                            @endif
                        </div>
                    </div>
                                ...
                                <div wire:ignore>
                                    ...
                                    <select class="form-select js-states" id="hotelId" name="hotelId" data-width="100%" wire:model.defer="hotelId">
                                        ...
                                        @foreach($hoteles as $hotel)
                                            <option value="{{$hotel->id}}">{{$hotel->nombre}} {{$hotel->ubicacion?->nombre}} {{$hotel->categoria?->nombre}}</option>
                                        @endforeach
                                    </select>
                                </div>
                            </div>
                    ...
                    @for($k=0; $k < $contDias; $k++)
                        ...
                        <div class="row">
                            ...
                            <div class="mb-3 col-md-2">
                                ...
                                @else
                                    @if(isset($img_principaltemplate[$k]))
                                        <a wire:click="verImagen('{{ asset($img_principaltemplate[$k]) }}')">
                                            <img src="{{asset($img_principaltemplate[$k])}}"  class="template">
                                        </a>
                                    @endif
                                @endif
                            </div>
...
<script>
    document.addEventListener('livewire:load', function () {
        ...
        $('#ubicacion_id').on('change', function (e) {
            var selectedValues = $(this).val();
            @this.set('ubicacion_id', selectedValues);
        ...
        $('#no_incluye').on('change', function (e) {
            var selectedValues = $(this).val();
            @this.set('no_incluye', selectedValues);
...
$(document).ready(function() {
    ...
    $('#descripcion').summernote('code', @this.get('descripcion'));
    window.livewire.on('refreshSummernote', () => {
        $('#descripcion').summernote('code', @this.get('descripcion'));
    });
});
...
$(document).ready(function() {
    ...
    $('#condicion').summernote('code', @this.get('condicion'));
    window.livewire.on('refreshSummernote', () => {
        $('#condicion').summernote('code', @this.get('condicion'));
    });
});
...
const inputElement3 = document.querySelector('#template0');
const inputElement4 = document.querySelector('#template1');
const inputElement5 = document.querySelector('#template2');
    ...
    server:{
        process:(fieldName,file,metadata,load,error,progress,abort,transfer,options)=>{
            if (file.type == 'image/heic') {
                ...
                // Envía el archivo convertido al servidor utilizando Livewire
                @this.upload('template.1',formData,load,error,progress)
            } else {
                // Si no es .heic, simplemente envía el archivo al servidor sin conversión
                @this.upload('template.1',file,load,error,progress)
            }
        },

        revert:(filename,load)=>{
            @this.removeUpload('template.1',filename,load)
        },
    ...
    server:{
        process:(fieldName,file,metadata,load,error,progress,abort,transfer,options)=>{
            if (file.type == 'image/heic') {
                ...
                // Envía el archivo convertido al servidor utilizando Livewire
                @this.upload('template.2',formData,load,error,progress)
            } else {
                // Si no es .heic, simplemente envía el archivo al servidor sin conversión
                @this.upload('template.2',file,load,error,progress)
            }
        },

        revert:(filename,load)=>{
            @this.removeUpload('template.2',filename,load)
        },
...
<script>
    ...
    document.addEventListener('livewire:load', function () {
        function initializeSelect() {
            $('[id^="nombrePrecio"]').each(function() {
                const id = $(this).attr('id').replace('nombrePrecio', '');
                $(this).select2();
            ...
            $('[id^="monedaPrecio"]').each(function() {
                const id = $(this).attr('id').replace('monedaPrecio', '');
                $(this).select2();
            ...
            $('[id^="incluye"]').each(function() {
                const id = $(this).attr('id').replace('incluye', '');
                $(this).select2();

                // Sincronizar con el modelo de Livewire
                $(this).on('change', function (e) {
                    @this.set('incluye.' + id, $(this).val());
                });
            });

            $('[id^="no_incluye"]').each(function() {
                const id = $(this).attr('id').replace('no_incluye', '');
                $(this).select2();

                // Sincronizar con el modelo de Livewire
                $(this).on('change', function (e) {
                    @this.set('no_incluye.' + id, $(this).val());
                });
            });
