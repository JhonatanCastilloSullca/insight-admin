...
<head>
    <title>{{ $reserva?->paquete?->titulo }}</title>
    <style>
        @page {
            width: 297mm;
            height: 167mm;
        ...
        .firstpage {
            ...
            height: 100%;
            width: 100%;
            background: url('{{ $reserva->paquete ? asset('storage/' . $reserva->paquete?->img_principal) : asset('storage/paquetes/default.png') }}') no-repeat center center;
            background-size: cover;
        }
        ...
        .secondpage {
            ...
            width: 100%;
            height: 100%;
            background: url('{{ $reserva->paquete ? asset('storage/' . $reserva->paquete?->img_secundario) : asset('storage/paquetes/default-secundario.png') }}') no-repeat center center;
            background-size: cover;
        }
...
<body>
    ...
    <div class="secondpage">
        <div class="text-second-page">
            <div class="contenido-second">
                <h4 class="h4-paquete">{{ $reserva->observacion }} :</h4>
                <h3 class="h3-paquete">
                    @foreach ($reserva->detallestoursTitulo() as $index => $detalle)
    ...
    @foreach ($reserva->detallestoursItinerario as $k => $detalle )
    ...
    <div class="container2">
        <div class="container3">
            <div class="text-container2">
                <h2>COTIZACION EN BASE A {{$cantAdultos > 0 ? $cantAdultos.' adulto' : ''}} {{$cantNiños > 0 ? $cantNiños.' niño' : ''}} TOTAL: {{ $reserva->totales[0]->moneda->id == 1 ? 'PEN '.$reserva->totales[0]->total:'USD '.$reserva->totales[0]->total}}</h2>
                @if($cantAdultos > 0)
                    <h3 class="costos-detalles">
                        Precio por persona adulto sin hoteles ni vuelos aereos:
                    </h3>
                    <div class="card-precio-medios">
                        <H4 class="text-card-precios">{{$reserva->totales[0]->moneda->id == 1 ? 'Nacional':'Extranjero'}} <br>{{ $reserva->totales[0]->moneda->id == 1 ? 'PEN '.($reserva->detallestours()->where('precio_id',1)->sum('precio')) : 'USD '.($reserva->detallestours()->where('precio_id',1)->sum('precio'))}}</H4>
                    </div>
                @endif
                @if($cantNiños > 0)
                    <h3 class="costos-detalles">
                        Precio por persona niño sin hoteles ni vuelos aereos:
                    </h3>
                    <div class="card-precio-medios">
                        <H4 class="text-card-precios">{{$reserva->totales[0]->moneda->id == 1 ? 'Nacional':'Extranjero'}} <br>{{ $reserva->totales[0]->moneda->id == 1 ? 'PEN '.($reserva->detallestours()->where('precio_id',2)->sum('precio')) : 'USD '.($reserva->detallestours()->where('precio_id',2)->sum('precio'))}}</H4>
                    </div>
                @endif
                @if(count($reserva->detalleshoteles)>0  || count($reserva->detallesvuelos)>0 )
                    @if($cantAdultos > 0)
                        <h3 class="costos-detalles">
                            Precio por persona adulto {{count($reserva->detalleshoteles)>0 ? 'con hoteles, ' : 'sin hoteles, '}} {{count($reserva->detallesvuelos)>0 ? 'con vuelos aereos' : 'sin vuelos aereos'}}:
                        </h3>
                        <div class="card-precio-medios">
                            <H4 class="text-card-precios">{{$reserva->totales[0]->moneda->id == 1 ? 'Nacional':'Extranjero'}} <br>{{ $reserva->totales[0]->moneda->id == 1 ? 'PEN '.number_format(($reserva->detallestours()->where('precio_id',1)->sum('precio') + $totalVuelos/($cantAdultos + $cantNiños) + $totalHoteles/($cantAdultos + $cantNiños)  ),2) : 'USD '.number_format(($reserva->detallestours()->where('precio_id',1)->sum('precio') + $totalVuelos/($cantAdultos + $cantNiños)  + $totalHoteles/($cantAdultos + $cantNiños) ),2)}}</H4>
                        </div>
                    @endif
                    @if($cantNiños > 0)
                        <h3 class="costos-detalles">
                            Precio por persona niño {{count($reserva->detalleshoteles)>0 ? 'con hoteles, ' : 'sin hoteles, '}} {{count($reserva->detallesvuelos)>0 ? 'con vuelos aereos nacionales' : 'sin vuelos aereos nacionales'}}:
                        </h3>
                        <div class="card-precio-medios">
                            <H4 class="text-card-precios">{{$reserva->totales[0]->moneda->id == 1 ? 'Nacional':'Extranjero'}} <br>{{ $reserva->totales[0]->moneda->id == 1 ? 'PEN '.number_format(($reserva->detallestours()->where('precio_id',2)->sum('precio') + $totalVuelos/($cantAdultos + $cantNiños)  + $totalHoteles/($cantAdultos + $cantNiños) ),2) : 'USD '.number_format(($reserva->detallestours()->where('precio_id',2)->sum('precio') + $totalVuelos/($cantAdultos + $cantNiños)  + $totalHoteles/($cantAdultos + $cantNiños) ),2)}}</H4>
                        </div>
                    @endif
                @endif
            </div>
